...
class SustainSplash extends FlxSprite
{
	...
	public function new():Void
	{
		...
		frames = Paths.getSparrowAtlas('holdCovers/holdCover-Vanilla');
	...
	override function update(elapsed)
	{
		...
		if (strumNote != null)
		{
			...
			alpha = 1; // Siempre alpha 1

			if (animation.curAnim != null && strumNote != null && strumNote.animation != null && strumNote.animation.curAnim != null)
			{
	...
	public function setupSusSplash(strum:StrumNote, daNote:Note, ?playbackRate:Float = 1):Void
	{
		...
		var tailArr = !daNote.isSustainNote ? daNote.tail : daNote.parent.tail;
		if (tailArr == null || tailArr.length == 0) return; // <-- Previene crash si no hay tail
	}
}
