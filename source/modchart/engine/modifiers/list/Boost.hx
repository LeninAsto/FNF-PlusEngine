...
class Boost extends Modifier {
	...
	override public function render(curPos:Vector3, params:ModifierParameters) {
		...
		final boost = getUnsafeLaneOverride(_boostID, _boostIDs[lane], player);
		if (boost != 0) {
		...
		final brake = getUnsafeLaneOverride(_brakeID, _brakeIDs[lane], player);
		...
		final wave = getUnsafeLaneOverride(_waveID, _waveIDs[lane], player);
