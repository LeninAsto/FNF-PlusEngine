...
class MaterialChip extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0, label:String = "Chip", ?chipType:ChipType = ASSIST, ?selected:Bool = false, ?onTap:Void->Void = null,
			?onDelete:Void->Void = null)
	{
		super(x, y);
		...
		labelText.setFormat(Paths.font('NotoSans-Medium.ttf'), LABEL_SIZE, MD3Theme.onSurfaceVariant, LEFT);
	...
	function drawXIcon(sprite:FlxSprite, size:Int):Void
	{
		...
		for (i in 0...(size - margin * 2))
		{
			...
			if (px1 >= 0 && px1 < size && py1 >= 0 && py1 < size)
				graphics.setPixel32(px1, py1, col);
			if (px2 >= 0 && px2 < size && py2 >= 0 && py2 < size)
				graphics.setPixel32(px2, py2, col);
		}
	}
	...
	function updateAppearance():Void
	{
		...
		if (!enabled)
		{
			...
			if (deleteIcon != null)
				deleteIcon.color = MD3Theme.disabledContentColor();
			if (checkMark != null)
				checkMark.visible = false;
			return;
		}
