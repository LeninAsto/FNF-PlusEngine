...
class FunkinLua
{
	...
	public function new(scriptName:String, ?context:LuaHostContext = null, ?autoCallOnCreate:Bool = true)
	{
		...
		addLocalCallback("setOnScripts", function(varName:String, arg:Dynamic, ?ignoreSelf:Bool = false, ?exclusions:Array<String> = null)
		{
			...
			if (game != null)
				...
				setOnSiblingScripts(varName, arg, exclusions);
		});
		addLocalCallback("setOnHScript", function(varName:String, arg:Dynamic, ?ignoreSelf:Bool = false, ?exclusions:Array<String> = null)
		{
			...
			if (game != null)
				...
				luaTrace('setOnHScript is only available in PlayState for Lua scripts right now.', false, false, FlxColor.YELLOW);
		});
		...
		{
			if (game == null)
				return warnPlayStateOnly('setHealth');
			game.health = value;
			return true;
		});
		...
		{
			if (game == null)
				return warnPlayStateOnly('addHealth');
			game.health += value;
			return true;
		});
		...
		{
			if (game == null)
				return warnPlayStateOnly('startCountdown');
			game.startCountdown();
			return true;
		});
		...
		{
			if (game == null)
				return warnPlayStateOnly('endSong');
			game.KillNotes();
			game.endSong();
			return true;
		});
		...
		{
			if (game == null)
				return warnPlayStateOnly('setCharacterX');
			switch (type.toLowerCase())
			{
				case 'dad' | 'opponent':
		...
		{
			if (game == null)
				return warnPlayStateOnly('setCharacterY');
			switch (type.toLowerCase())
			{
				case 'dad' | 'opponent':
		...
		Lua_helper.add_callback(lua, "debugPrint", function(text:Dynamic = '', color:String = 'WHITE')
		{
			...
			if (PlayState.instance != null)
				PlayState.instance.addTextToDebug(text, flxColor);
			else
				luaTrace(Std.string(text), true, false, flxColor);
		});
	...
	{
		if (context != null && context.scriptList != null)
	...
	function callOnSiblingScripts(funcName:String, ?args:Array<Dynamic> = null, ?ignoreStops:Bool = false, ?excludeScripts:Array<String> = null,
			?excludeValues:Array<Dynamic> = null):Dynamic
	{
		...
		for (script in getSiblingScripts())
		{
			if (script == null || excludeScripts.contains(script.scriptName))
				continue;
			...
			{
				ret = value;
				if (!ignoreStops && LuaUtils.isStop(value))
					return value;
			}
		}
		return ret;
