...
class Conductor
{
	...
	function get_bpm():Float
	{
		...
		if (PlayState.instance != null && PlayState.instance.startingSong)
		{
			...
			{
				if (PlayState.instance.startTimestamp >= timeChanges[i].timeStamp)
	...
	static function set_instance(instance:Conductor):Conductor
	{
		...
		if (Conductor._instance != null)
			clearSingleton(Conductor._instance);

		Conductor._instance = instance;

		if (Conductor._instance != null)
			setupSingleton(Conductor._instance);
	...
	public function update(?songPos:Float, applyOffsets:Bool = true, forceDispatch:Bool = false):Void
	{
		...
		if (this == Conductor.instance)
			@:privateAccess SongSequence.update.dispatch();
	}
	...
	public function mapTimeChanges(songTimeChanges:Array<SongTimeChange>):Void
	{
		...
		{
			var fallback:SongTimeChange = new SongTimeChange(0, Constants.DEFAULT_BPM);
			fallback.beatTime = 0.0;
			timeChanges.push(fallback);
			currentTimeChange = fallback;
			log(' WARNING '.warning() + 'No time changes provided; using ${Constants.DEFAULT_BPM} BPM fallback.');
