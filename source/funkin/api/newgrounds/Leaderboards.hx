...
class Leaderboards
{
	...
	public static function requestScores(leaderboard:Leaderboard, ?params:RequestScoresParams)
	{
		...
		if ((params?.useCurrentUser ?? false) && NewgroundsClient.instance.isLoggedIn())
			user = NewgroundsClient.instance.user;
				...
				switch (outcome)
				{
					case SUCCESS:
						...
						if (params != null && params.onComplete != null)
							params.onComplete(leaderboardData.scores);

					case FAIL(error):
						trace(' NEWGROUNDS '.bold().bg_orange() + ' ERROR '.error() + ' Failed to fetch scores!');
						trace(' ERROR '.error() + error);
						if (params != null && params.onFail != null)
							params.onFail();
				}
			});
	}
