...
class PlayerRegistry extends BaseRegistry<PlayableCharacter, PlayerData, PlayerEntryParams> implements ISingleton implements DefaultRegistryImpl
{
  ...
  public override function loadEntries():Void
  {
    ...
    refreshOwnedCharacterIds();
  }

  public function refreshOwnedCharacterIds():Void
  {
    ...
    inferOwnedCharacterIdsFromCharacterData();

    log('Loaded ${countEntries()} playable characters with ${ownedCharacterIds.size()} associations.');
  }

  function inferOwnedCharacterIdsFromCharacterData():Void
  {
    for (characterId in CharacterDataParser.listCharacterIds())
    {
      if (ownedCharacterIds.exists(characterId)) continue;

      var ownerId:Null<String> = inferOwnerId(characterId);
      if (ownerId != null) ownedCharacterIds.set(characterId, ownerId);
    }
  }

  function inferOwnerId(characterId:String):Null<String>
  {
    var characterData = CharacterDataParser.fetchCharacterData(characterId);
    var iconId:Null<String> = characterData?.healthIcon?.id;

    if (iconId != null && fetchEntry(iconId) != null) return iconId;

    for (playerId in listEntryIds())
    {
      if (characterId == playerId || StringTools.startsWith(characterId, '$playerId-')) return playerId;
    }
