...
class PlayerRegistry extends BaseRegistry<PlayableCharacter, PlayerData, PlayerEntryParams> implements ISingleton implements DefaultRegistryImpl
{
	...
	function inferOwnedCharacterIdsFromCharacterData():Void
	{
		...
		{
			if (ownedCharacterIds.exists(characterId))
				continue;

			var ownerId:Null<String> = inferOwnerId(characterId);
			if (ownerId != null)
				ownedCharacterIds.set(characterId, ownerId);
		}
	}
	...
	function inferOwnerId(characterId:String):Null<String>
	{
		...
		if (iconId != null && fetchEntry(iconId) != null)
			return iconId;

		for (playerId in listEntryIds())
		{
			if (characterId == playerId || StringTools.startsWith(characterId, '$playerId-'))
				return playerId;
		}
	...
	public function hasNewCharacter():Bool
	{
		...
		for (charId in listEntryIds())
		{
			...
			if (!player.isUnlocked())
				continue;
			if (charactersSeen.contains(charId))
				continue;
