...
class SongNoteDataUtils
{
				...
				for (j in (i + 1)...chunk.length)
				{
					...
					if (doNotesStack(noteI, noteJ, threshold))
					{
						if (!stackedNotes.fastContains(noteI))
						{
							...
							if (overlapped != null && !overlapped.contains(noteI))
								overlapped.push(noteI);
						}
	...
	{
		if (lhs == null || rhs == null || rhs.length == 0)
			return lhs;
		if (lhs.length == 0)
			return rhs;
	...
	{
		if (noteA.data != noteB.data)
			return false;
		else if (threshold == 0)
			return Math.ffloor(Math.abs(noteA.time - noteB.time)) < 1;
