...
class FNFLegacyImporter
{
	...
	public static function migrateChartData(songData:FNFLegacyData, difficulty:String = 'normal'):SongChartData
	{
		...
		switch (songData.song.speed)
		{
			...
			case Right(speeds):
				if (speeds.easy != null)
					songChartData.scrollSpeed.set('easy', speeds.easy);
				if (speeds.normal != null)
					songChartData.scrollSpeed.set('normal', speeds.normal);
				if (speeds.hard != null)
					songChartData.scrollSpeed.set('hard', speeds.hard);
		}
	...
	static function rebuildEventData(songData:FNFLegacyData):Array<SongEventData>
	{
		...
		switch (songData.song.notes)
		{
			...
			case Right(difficulties):
				...
				if (difficulties.hard != null)
					noteSections = difficulties.hard;
				if (difficulties.easy != null)
					noteSections = difficulties.easy;
		}
	...
	static function rebuildTimeChanges(songData:FNFLegacyData):Array<SongTimeChange>
	{
		...
		for (noteSection in noteSections)
		{
			if (noteSection.changeBPM ?? false)
			{
				...
				if (firstNote != null)
					result.push(new SongTimeChange(firstNote.time, noteSection.bpm ?? Constants.DEFAULT_BPM));
			}
		}
	...
	static function migrateNoteSections(input:Array<LegacyNoteSection>):Array<SongNoteData>
	{
		...
		for (section in input)
		{
			...
			for (note in section.sectionNotes)
			{
				...
				if (noteData < 0)
					continue; // Exclude Psych event notes.
				if (noteData > (STRUMLINE_SIZE * 2))
					noteData = noteData % (2 * STRUMLINE_SIZE); // Handle other engine event notes.
