...
import funkin.input.Controls as FunkinControls;
...
class PreciseInputManager extends FlxKeyManager<FlxKey, PreciseInputList>
{
  ...
  var _deviceBinds:Map<FlxGamepad,
    {
      onButtonDown:LimeGamepadButton->Void,
      onButtonUp:LimeGamepadButton->Void
    }>;
  ...
  public function new()
  {
    ...
    FlxG.stage.application.window.onKeyDown.add(handleKeyDownFallback);
    FlxG.stage.application.window.onKeyUp.add(handleKeyUpFallback);
  ...
  public static function getKeysForDirection(controls:FunkinControls, noteDirection:NoteDirection)
  {
  ...
  public static function getButtonsForDirection(controls:FunkinControls, noteDirection:NoteDirection)
  {
    return switch (noteDirection)
    {
   ...
   */
  public function initializeKeys(controls:FunkinControls):Void
  {
    clearKeys();
  ...
  public function initializeButtons(controls:FunkinControls, gamepad:FlxGamepad):Void
  {
    clearButtons();

    var limeGamepad = FlxGamepadUtil.getLimeGamepad(gamepad);
    if (limeGamepad == null) return;

    var callbacks = {
      onButtonDown: handleButtonDownFallback.bind(gamepad),
      onButtonUp: handleButtonUpFallback.bind(gamepad)
    };
    limeGamepad.onButtonDown.add(callbacks.onButtonDown);
    limeGamepad.onButtonUp.add(callbacks.onButtonUp);
  ...
  function handleKeyDownFallback(keyCode:KeyCode, modifier:KeyModifier):Void
  {
    handleKeyDown(keyCode, modifier, getCurrentTimestamp());
  }

  function handleKeyUpFallback(keyCode:KeyCode, modifier:KeyModifier):Void
  {
    handleKeyUp(keyCode, modifier, getCurrentTimestamp());
  }
  ...
  function handleButtonDownFallback(gamepad:FlxGamepad, button:LimeGamepadButton):Void
  {
    handleButtonDown(gamepad, button, getCurrentTimestamp());
  }

  function handleButtonUpFallback(gamepad:FlxGamepad, button:LimeGamepadButton):Void
  {
    handleButtonUp(gamepad, button, getCurrentTimestamp());
  }
  ...
  function clearButtons():Void
  {
    ...
    for (gamepad in _deviceBinds.keys())
    {
      ...
      var limeGamepad = FlxGamepadUtil.getLimeGamepad(gamepad);
      limeGamepad.onButtonDown.remove(callbacks.onButtonDown);
      limeGamepad.onButtonUp.remove(callbacks.onButtonUp);
    }
    _deviceBinds.clear();
  }
  ...
  {
    // Keyboard
    FlxG.stage.application.window.onKeyDown.remove(handleKeyDownFallback);
    FlxG.stage.application.window.onKeyUp.remove(handleKeyUpFallback);
