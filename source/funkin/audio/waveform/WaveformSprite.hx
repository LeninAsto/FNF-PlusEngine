...
class WaveformSprite extends MeshRender
{
	...
	function drawWaveform():Void
	{
		...
		if (waveformData == null || !this.visible)
			return;
		...
		if (pixelsPerIndex >= 1.0)
		{
			...
			for (i in startIndex...endIndex)
			{
				...
				var isBeforeClipRect:Bool = (clipRect != null)
					&& ((orientation == HORIZONTAL) ? pixelPos < clipRect.x : pixelPos < clipRect.y);
				...
				if (sampleMaxSize < minWaveformSize)
					sampleMaxSize = minWaveformSize;
				var sampleMinSize:Float = sampleMin * (orientation == HORIZONTAL ? this.height : this.width) / 2;
				if (sampleMinSize > -minWaveformSize)
					sampleMinSize = -minWaveformSize;
				...
				if (clipRect != null)
				{
					...
					{
						vertexTopIndex = buildClippedVertex(pixelPos, vertexTopY, topLeftVertexIndex, topRightVertexIndex, bottomLeftVertexIndex,
					...
					{
						vertexTopIndex = buildClippedVertex(vertexTopY, pixelPos, topLeftVertexIndex, topRightVertexIndex, bottomLeftVertexIndex,
							bottomRightVertexIndex);
