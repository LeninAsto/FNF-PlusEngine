...
class FunkinSound extends FlxSound implements ICloneable<FunkinSound>
{
	...
	function get_waveformData():WaveformData
	{
		if (_waveformData == null)
		{
			...
			if (_waveformData == null)
				throw 'Could not interpret waveform data!';
		}
	...
	public static function playMusic(key:String, params:FunkinSoundPlayMusicParams):Bool
	{
		...
		if (params?.mapTimeChanges ?? true)
		{
			...
			if (songMusicData != null)
			{
				...
				if (songMusicData.looped != null && params.loop == null)
					params.loop = songMusicData.looped;
			}
			else
			{
		...
		{
			var music = FunkinSound.loadPartial(pathToUse, params.partialParams?.start ?? 0.0, params.partialParams?.end ?? 1.0,
			...
			if (music != null)
			{
				...
				music.future.onComplete(function(partialMusic:Null<FunkinSound>)
				{
					...
					if (FlxG.sound.music != null && params.onLoad != null)
						params.onLoad();
				});
		...
		{
			var music = FunkinSound.load(pathToUse, params?.startingVolume ?? 1.0, params.loop ?? true, false, true, params.persist ?? false,
				params.onComplete);
			if (music != null)
			{
				setMusic(music);
	 ...
	 */
	static function resolveVSliceSoundPath(path:String):String
	{
		...
		if (path.indexOf(':') == -1)
		{
			if (!path.startsWith('assets/'))
			{
				...
				candidates.push('preload:$path');
				candidates.push('preload:assets/$path');
				candidates.push('shared:$path');
				candidates.push('shared:assets/shared/$path');
				candidates.push('assets/shared/$path');
				candidates.push('assets/funkin/shared/$path');
				candidates.push('shared:assets/funkin/shared/$path');
			}
			else
			{
				var nakedPath:String = path.substr('assets/'.length);
				candidates.push(nakedPath);
				candidates.push('preload:$nakedPath');
				candidates.push('preload:$path');
				candidates.push('shared:$nakedPath');
				candidates.push('shared:$path');
			}
		}
		...
		{
			if (openfl.utils.Assets.exists(candidate, AssetType.SOUND)
				|| openfl.utils.Assets.exists(candidate, AssetType.MUSIC)
				|| openfl.utils.Assets.exists(candidate))
			{
				return candidate;
			}
	...
	public static function stopAllAudio(musicToo:Bool = false, persistToo:Bool = false):Void
	{
		for (sound in pool)
		{
			if (sound == null)
				continue;
			if (!persistToo && sound.persist)
				continue;
			if (!musicToo && sound == FlxG.sound.music)
				continue;
			sound.destroy();
		}
	}
