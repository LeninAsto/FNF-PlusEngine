...
class FunkinMemory
{
	...
	public static function getCachedGraphic(path:String):Null<FlxGraphic>
	{
		...
		if (previousCachedTextures.exists(path))
			return previousCachedTextures.get(path); // just in case
	...
	public static function cacheSound(key:String):Void
	{
		...
		var sound:Null<Sound> = loadSoundForCache(key);
		if (sound == null)
			return;
		else
			currentCachedSounds.set(key, sound);
	...
	static function loadSoundForCache(key:String):Null<Sound>
	{
		if (!Assets.exists(key, SOUND))
		{
			trace('[FunkinMemory] Missing sound cache target: ${key}');
			return null;
		}

		try
		{
			return Assets.getSound(key, true);
		}
		catch (e)
		{
			trace('[FunkinMemory] Failed to cache sound ${key}: ${e}');
			return null;
		}
	}
