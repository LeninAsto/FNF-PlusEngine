...
class Assets implements ConsoleClass
{
  ...
  static function namespacedVariants(path:String):Array<String>
  {
    if (path == null || path.indexOf('assets/funkin/') != -1) return [];
    ...
    if (colonIndex != -1)
    {
      prefix = path.substr(0, colonIndex + 1);
      assetPath = path.substr(colonIndex + 1);
    }

    if (assetPath.startsWith('assets/'))
      variants.push(prefix + 'assets/funkin/' + assetPath.substr('assets/'.length));

    if (prefix.length > 0 && assetPath.startsWith('assets/'))
      variants.push('assets/funkin/' + assetPath.substr('assets/'.length));

    return variants;
  }

  public static function resolvePath(path:String, ?type:openfl.utils.AssetType):String
  {
    if (openfl.utils.Assets.exists(path, type)) return path;

    for (variant in namespacedVariants(path))
    {
      if (openfl.utils.Assets.exists(variant, type)) return variant;
    }
  ...
  {
    return openfl.utils.Assets.getPath(resolvePath(path));
  }
  ...
  {
    return openfl.utils.Assets.getBytes(resolvePath(path, BINARY));
  }
  ...
  {
    return openfl.utils.Assets.loadBytes(resolvePath(path, BINARY));
  }
  ...
  {
    return openfl.utils.Assets.getText(resolvePath(path, TEXT));
  }
  ...
  {
    return openfl.utils.Assets.loadText(resolvePath(path, TEXT));
  }
   ...
   */
  public static function getSound(path:String, useCache:Bool = true):openfl.media.Sound
  {
    return openfl.utils.Assets.getSound(resolvePath(path, SOUND), useCache);
  }
  ...
  {
    return openfl.utils.Assets.loadSound(resolvePath(path, SOUND));
  }
  ...
  {
    return openfl.utils.Assets.getMusic(resolvePath(path, MUSIC));
  }
  ...
  {
    return openfl.utils.Assets.loadMusic(resolvePath(path, MUSIC));
  }
  ...
  {
    return openfl.utils.Assets.getBitmapData(resolvePath(path, IMAGE), useCache);
  }
  ...
  {
    return openfl.utils.Assets.loadBitmapData(resolvePath(path, IMAGE));
  }
  ...
  public static function exists(path:String, ?type:openfl.utils.AssetType):Bool
  {
    ...
    for (variant in namespacedVariants(path))
      if (openfl.utils.Assets.exists(variant, type)) return true;
    return false;
  }
