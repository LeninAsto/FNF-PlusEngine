...
class IntervalShake implements IFlxDestroyable
{
	...
	function shakeProgress(timer:FlxTimer):Void
	{
		...
		if (axes.x)
			object.x = initialOffset.x + FlxG.random.float((-curIntensity) * object.width, (curIntensity) * object.width);
		if (axes.y)
			object.y = initialOffset.y + FlxG.random.float((-curIntensity) * object.width, (curIntensity) * object.width);
