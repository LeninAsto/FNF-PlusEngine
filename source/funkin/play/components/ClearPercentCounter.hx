...
class ClearPercentCounter extends FlxTypedSpriteGroup<FlxSprite>
{
  ...
  public function new(x:Float, y:Float, startingNumber:Int = 0, isSmall:Bool = false)
  {
    ...
    var clearPercentText:FunkinSprite = FunkinSprite.create(0, 0, 'resultScreen/clearPercent/clearPercentText${isSmall ? 'Small' : ''}');
    clearPercentText.x = isSmall ? 40 : 0;
  ...
  function drawNumbers():Void
  {
    ...
    for (ind => num in seperatedScore)
    {
      ...
      var digitSize = isSmall ? 32 : 72;
      var digitHeightOffset = isSmall ? -4 : 0;
      ...
      if (digitIndex >= members.length)
      {
        ...
        var numb:ClearPercentNumber = new ClearPercentNumber(xPos, yPos, num, variant, this.isSmall);
