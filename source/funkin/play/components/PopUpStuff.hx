...
class PopUpStuff extends FlxTypedGroup<FunkinSprite>
{
  ...
  var plusRatingOffsets:Array<Float> = [0, 0];
  var plusComboOffsets:Array<Float> = [0, 0];
  ...
  public function displayRating(daRating:Null<String>)
  {
    ...
    rating.x += offsets[0];
    rating.y += offsets[1];
    rating.x += plusRatingOffsets[0];
    rating.y += plusRatingOffsets[1];
  ...
  public function displayCombo(combo:Int = 0):Void
  {
    ...
    for (digit in seperatedScore)
    {
      ...
      numScore.x += plusComboOffsets[0];
      numScore.y += plusComboOffsets[1];
  ...
  public function setPlusComboOffsets(comboOffset:Array<Int>):Void
  {
    plusRatingOffsets = [safeOffset(comboOffset, 0), -safeOffset(comboOffset, 1)];
    plusComboOffsets = [safeOffset(comboOffset, 2), -safeOffset(comboOffset, 3)];
  }

  static inline function safeOffset(offsets:Array<Int>, index:Int):Int
  {
    return (offsets != null && index >= 0 && index < offsets.length) ? offsets[index] : 0;
  }
}
