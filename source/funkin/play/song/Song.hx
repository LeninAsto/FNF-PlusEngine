...
class Song implements IPlayStateScriptedClass implements IRegistryEntry<SongMetadata>
{
  ...
  public function new(id:String, ?params:SongParams)
  {
    ...
    if (this.variation != null && this.variation != Constants.DEFAULT_VARIATION && !_metadata.exists(this.variation))
    {
      if (validateVariationId(this.variation))
      {
        var requestedMeta:Null<SongMetadata> = fetchVariationMetadata(id, this.variation);
        if (requestedMeta != null)
        {
          _metadata.set(requestedMeta.variation, requestedMeta);
          log('Loaded requested variation: ${this.variation}');
        }
        else
        {
          FlxG.log.warn('[SONG] Failed to load requested variation metadata (${id}:${this.variation}), is the path correct?');
          log('FAILED to load requested variation: ${this.variation}');
        }
      }
    }
  ...
  public function ensureVariationLoaded(variation:String):Bool
  {
    if (variation == null || variation.length == 0) variation = Constants.DEFAULT_VARIATION;
    if (_metadata.exists(variation)) return true;
    if (variation == Constants.DEFAULT_VARIATION) return false;
    if (!validateVariationId(variation)) return false;

    var variMeta:Null<SongMetadata> = fetchVariationMetadata(id, variation);
    if (variMeta == null) return false;

    _metadata.set(variMeta.variation, variMeta);
    populateDifficulties();
    log('Loaded variation on demand: $variation');
    return true;
  }
...
class SongDifficulty
{
  ...
  {
    var instPath:String = getInstPath(instId);
    trace(' SONG '.bold().bg_note_down() + ' Playing instrumental track "$instPath" for song "${song.id}"');
    FlxG.sound.music = FunkinSound.load(instPath, volume, looped, false, true, false, null, null, true);
