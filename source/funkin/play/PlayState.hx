...
class PlayState extends MusicBeatSubState
{
  ...
  **/
  ...
  public function new(?params:PlayStateParams)
  {
    ...
    // Strumlines
    var strumScrollSpeed:Float = VSlicePreferencesBridge.scrollSpeed(currentChart?.scrollSpeed);
    playerStrumline = new Strumline(noteStyle, !isBotPlayMode, strumScrollSpeed);
  ...
  public override function create():Void
  {
    ...
    if (!ControlsHandler.hasExternalInputDevice)
    {
      ...
      if (hitbox != null)
      {
        ...
        if (Preferences.controlsScheme == FunkinHitboxControlSchemes.Arrows && ClientPrefs.data.mobileReceptorAlign)
        {
  ...
  {
    final isDownscroll:Bool = #if mobile (Preferences.controlsScheme == FunkinHitboxControlSchemes.Arrows && ClientPrefs.data.mobileReceptorAlign
  ...
  function initPlusVersionText():Void
  {
    if (!ClientPrefs.data.versionTextOnGameplay) return;

    var songLabel:String = currentChart?.songName ?? currentSong?.id ?? '???';
    var difficultyLabel:String = currentDifficulty != null ? currentDifficulty.replace('-', ' ').toTitleCase() : '???';
    var versionStr:String = 'PlE v${states.MainMenuState.plusEngineVersion} | $songLabel ($difficultyLabel)';
  ...
  function showPlusVersionText():Void
  {
  ...
  function doPlusVersionTextBump():Void
  {
    if (versionText == null) return;
  ...
  function initStrumlines():Void
  {
    ...
    #if mobile
    if (Preferences.controlsScheme == FunkinHitboxControlSchemes.Arrows && ClientPrefs.data.mobileReceptorAlign && !ControlsHandler.hasExternalInputDevice)
    {
      initNoteHitbox();
    }
    #end
  ...
  function initPopups():Void
  {
    ...
    comboPopUps.setPlusComboOffsets(ClientPrefs.data.comboOffset);
  ...
  function initPlusKeyViewer():Void
  {
    if (!ClientPrefs.data.showKeyViewer) return;

    keyViewer = new KeyViewer(0, 0, this);
    keyViewer.visible = !ClientPrefs.data.hideHud;
    keyViewer.cameras = [camHUD];
    keyViewer.zIndex = 10001;
    add(keyViewer);
  }
  ...
  function cleanupPreciseInputs():Void
  {
  ...
  function onKeyPress(event:PreciseInputEvent):Void
  {
    ...
    keyViewer?.keyPressed(noteDirectionToKeyViewerIndex(event.noteDirection));
  ...
  {
    keyViewer?.keyReleased(noteDirectionToKeyViewerIndex(event.noteDirection));

    // Do the minimal possible work here.
    inputReleaseQueue.push(event);
  }

  inline function noteDirectionToKeyViewerIndex(direction:NoteDirection):Int
  {
    return cast direction;
  }

  function pulseKeyViewer(direction:NoteDirection):Void
  {
    if (keyViewer == null) return;

    var keyIndex:Int = noteDirectionToKeyViewerIndex(direction);
    keyViewer.keyPressed(keyIndex);
    new FlxTimer().start(Conductor.instance.stepLengthMs / 1000 * 0.5, function(_)
    {
  ...
  function processNotes(elapsed:Float):Void
  {
    ...
    for (note in opponentStrumline.notes.members)
    {
      ...
      if (r.botplayHit)
      {
        ...
        if (VSlicePreferencesBridge.opponentDrain() && !isPracticeMode && health > Constants.HEALTH_MIN + 0.1)
        {
          health -= 0.02 * Constants.HEALTH_MAX;
        }
    ...
    for (note in playerStrumline.notes.members)
    {
      ...
      if (r.botplayHit)
      {
        ...
        if (isBotPlayMode && !note.isHoldNote) pulseKeyViewer(note.noteData.getDirection());
    ...
    for (holdNote in playerStrumline.holdNotes.members)
    {
      ...
      if (holdNote.missedNote && !holdNote.handledMiss)
      {
        ...
        if (!isBotPlayMode && holdNote.scoreable)
        {
          if (VSlicePreferencesBridge.noDropPenalty()) continue;
  ...
  function onNoteMiss(note:NoteSprite, playSound:Bool = false, healthChange:Float):Void
  {
    ...
    if (VSlicePreferencesBridge.instakillOnMiss() && !isPracticeMode)
    {
      health = Constants.HEALTH_MIN;
    }
  ...
  {
    if (VSlicePreferencesBridge.perfectMode() && daRating != 'sick' && daRating != '' && !isPracticeMode)
    {
      health = Constants.HEALTH_MIN;
    }
    ...
    if (healthChange > 0)
    {
      healthChange *= VSlicePreferencesBridge.healthGain();
    }
    else if (healthChange < 0)
    {
      healthChange *= VSlicePreferencesBridge.healthLoss();
    }
  ...
  {
    if (shouldReturnDirectlyToPlusFreeplay())
    {
      returnToPlusFreeplay();
      return;
    }
