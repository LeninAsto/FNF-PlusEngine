...
class PauseSubState extends MusicBeatSubState
{
  ...
  function changeSelection(change:Int = 0):Void
  {
    ...
    for (entryIndex in 0...currentMenuEntries.length)
    {
      ...
      var text:Alphabet = entry.sprite;
  ...
  function clearAndAddMenuEntries():Void
  {
    ...
    for (entry in currentMenuEntries)
    {
      ...
      else
      {
        ...
        var text:Alphabet = new Alphabet(110, yPos, translatePauseText(entry.text), true);
        ...
        var text:Alphabet = new Alphabet(0, yPos, translatePauseText(entry.text), true);
        text.scrollFactor.set(0, 0);
        text.alpha = 0;
        text.setScale(0.9);
        menuEntryText.add(text);
  ...
  static function translatePauseText(text:String):String
  {
    var exact:String = Language.getPhrase('pause_$text', text);
    if (exact != text) return exact;

    var key:String = text.toLowerCase().split(' ').join('_').split("'").join('');
    return Language.getPhrase('pause_$key', text);
  }
  ...
  static function quitToMenu(state:PauseSubState):Void
  {
    ...
    var targetState:funkin.ui.transition.stickers.StickerSubState->FlxState = (PlayStatePlaylist.isStoryMode) ? (sticker) ->
      new states.StoryMenuState() : (sticker) -> states.FreeplayStateSelector.create();
