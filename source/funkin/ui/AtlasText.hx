...
class AtlasText extends FlxTypedSpriteGroup<AtlasChar>
{
  ...
  inline function hasAtlas():Bool return font.atlas != null;
    ...
    for (splitStr in str.split(""))
    {
      switch (splitStr)
      {
        ...
        case char:
          ...
          if (group.members.length <= charCount) charSprite = new AtlasChar(0, 0, atlas, char, font.prefixSuffix, font.charScale);
          else
          {
            charSprite = group.members[charCount];
            charSprite.revive();
            charSprite.prefixSuffix = font.prefixSuffix;
            charSprite.charScale = font.charScale;
  ...
  function appendFallbackText(str:String):Void
  {
    var charCount:Int = group.countLiving();
    var xPos:Float = 0;
    var yPos:Float = 0;
    ...
    for (splitStr in str.split(""))
    {
      switch (splitStr)
      {
        ...
        case char:
          var charSprite:AtlasChar;
          if (group.members.length <= charCount) charSprite = new AtlasChar(0, 0, null, char, "", 1);
          else
          {
            charSprite = group.members[charCount];
            charSprite.revive();
            charSprite.prefixSuffix = "";
            charSprite.charScale = 1;
            charSprite.char = char;
            charSprite.alpha = 1;
          }
          charSprite.x = xPos;
  ...
  public function getWidth():Int
  {
    ...
    for (char in this.text.split(""))
    {
      switch (char)
      {
        ...
        case char:
          var sprite = new AtlasChar(0, 0, hasAtlas() ? atlas : null, char, font.prefixSuffix, font.charScale);
          sprite.revive();
          sprite.char = char;
          sprite.alpha = 1;
...
class AtlasChar extends FlxSprite
{
  ...
  public function new(x = 0.0, y = 0.0, atlas:Null<FlxAtlasFrames>, char:String, prefixSuffix:String = "", charScale:Float = 1)
  {
    super(x, y);
    frames = atlas;
  ...
  function set_char(value:String):String
  {
    ...
    if (frames != null)
    {
      animation.addByPrefix('anim', getAnimPrefix(value) + prefixSuffix, 24);
      if (!animation.exists('anim') && prefixSuffix != "") animation.addByPrefix('anim', getAnimPrefix(value), 24);
      if (animation.exists('anim')) animation.play('anim');
      scale.set(charScale, charScale);
    }
    else
    {
      makeGraphic(value == " " ? 20 : 22, 36, 0xFFFFFFFF);
      scale.set(1, 1);
    }
...
private class AtlasFontData
{
  ...
  public function new(name:AtlasFont)
  {
    ...
    if (atlas == null)
    {
      atlas = Paths.getSparrowAtlas('alphabet');
      if (atlas == null)
      {
        FlxG.log.warn('Could not find font atlas for font "${name}". Falling back to simple blocks.');
        maxHeight = 36;
        return;
      }

      prefixSuffix = name == AtlasFont.BOLD ? " bold" : " normal";
      charScale = name == AtlasFont.BOLD ? 0.42 : 0.38;
    }
    ...
    {
      maxHeight = Math.max(maxHeight, frame.frame.height * charScale);
