...
class OptionsState extends MusicBeatState
{
  ...
  public static var requestedInitialPage:Null<String> = null;
  public static var returnToPlusOptions:Bool = false;

  var optionsCodex:Codex<OptionsMenuPageName>;

  public var drumsBG:FunkinSound;
  var appliedRequestedInitialPage:Bool = false;
  var plusReturnHookInstalled:Bool = false;
  ...
  override function update(elapsed:Float):Void
  {
    ...
    @:privateAccess
    if (!appliedRequestedInitialPage && requestedInitialPage != null && optionsCodex != null && optionsCodex.pages.exists(cast requestedInitialPage))
    {
      appliedRequestedInitialPage = true;
      optionsCodex.switchPage(cast requestedInitialPage);
      installPlusReturnHook(cast requestedInitialPage);
      requestedInitialPage = null;
    }
  }

  function installPlusReturnHook(pageId:OptionsMenuPageName):Void
  {
    if (!returnToPlusOptions || plusReturnHookInstalled || optionsCodex == null) return;

    @:privateAccess
    var targetPage = optionsCodex.pages.get(pageId);
    if (targetPage == null) return;

    plusReturnHookInstalled = true;
    targetPage.onExit.add(returnToPlusOptionsState);
  }
  ...
  {
    if (returnToPlusOptions)
    {
      returnToPlusOptionsState();
      return;
    }
  ...
  function returnToPlusOptionsState():Void
  {
    if (!returnToPlusOptions) return;

    returnToPlusOptions = false;
    requestedInitialPage = null;
    if (optionsCodex != null && optionsCodex.currentPage != null) optionsCodex.currentPage.enabled = false;
