...
typedef StickerSubStateParams =
{
  ...
  /**
   * Re-open a second sticker substate after the target state switch so engines
   * that do not accept the official sticker constructor can still play the
   * sticker-out animation.
   */
...
class StickerSubState extends MusicBeatSubState
{
  ...
  public function new(params:StickerSubStateParams):Void
  {
    ...
    this.playOutOnTarget = params?.playOutOnTarget ?? false;
    this.stickerPackId = params?.stickerPack ?? Constants.DEFAULT_STICKER_PACK;
    this.stickerPack = resolveStickerPack(this.stickerPackId);
  ...
  {
    var targetStickerPack:Null<StickerPack> = StickerRegistry.instance.fetchEntry(id);
    if (targetStickerPack != null && targetStickerPack.getStickers().length > 0) return targetStickerPack;

    var defaultStickerPack:Null<StickerPack> = StickerRegistry.instance.fetchEntry(Constants.DEFAULT_STICKER_PACK);
    if (defaultStickerPack != null && defaultStickerPack.getStickers().length > 0) return defaultStickerPack;

    for (entryId in StickerRegistry.instance.listEntryIds())
    {
      var fallbackStickerPack:Null<StickerPack> = StickerRegistry.instance.fetchEntry(entryId);
      if (fallbackStickerPack != null && fallbackStickerPack.getStickers().length > 0) return fallbackStickerPack;
    }
  ...
  function playRandomStickerSound():Void
  {
    ...
    if (daSound == null || daSound.length == 0) return;
