...
class RuntimePostEffectShader extends FlxRuntimeShader
{
  ...
  public function new(fragmentSource:String = null, glVersion:String = null)
  {
    ...
    available = hasShaderField("uScreenResolution") && hasShaderField("uCameraBounds") && hasShaderField("uFrameBounds");
    updateViewDefaults();
  }
  ...
  {
    setFloatArraySafe("uScreenResolution", [screenWidth, screenHeight]);
    setFloatArraySafe("uCameraBounds", [camera.viewLeft, camera.viewTop, camera.viewRight, camera.viewBottom]);
  }

  public function updateFrameInfo(frame:FlxFrame)
  {
    // NOTE: uv.right is actually the right pos and uv.bottom is the bottom pos
    setFloatArraySafe("uFrameBounds", [frame.uv.left, frame.uv.top, frame.uv.right, frame.uv.bottom]);
  }

  public function hasShaderField(name:String):Bool
  {
    try
    {
      return data != null && Reflect.field(data, name) != null;
    }
    catch (error)
    {
      return false;
  ...
  public function validateShaderFields(fields:Array<String>):Bool
  {
    available = true;

    for (field in fields)
    {
      if (!hasShaderField(field))
      {
        available = false;
        break;
      }
  ...
  function updateViewDefaults():Void
  {
    setFloatArraySafe("uScreenResolution", [FlxG.width, FlxG.height]);
    setFloatArraySafe("uCameraBounds", [0, 0, FlxG.width, FlxG.height]);
    setFloatArraySafe("uFrameBounds", [0, 0, FlxG.width, FlxG.height]);
  }

  function setFloatArraySafe(name:String, value:Array<Float>):Void
  {
    try
    {
