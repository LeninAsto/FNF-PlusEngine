...
class Save implements ConsoleClass
{
  ...
  public static function load():Save
  {
    ...
    // Bind save data.
    final loadedSave:Save = try
    {
      loadFromSlot(Constants.BASE_SAVE_SLOT);
    }
    catch (e:Dynamic)
    {
      trace('[SAVE] Failed to load/migrate save data, using default VSlice save data: ${Std.string(e)}');
      new Save(Save.getDefaultData());
    }
  ...
  static function loadFromSlot(slot:Int):Save
  {
    ...
    var saveData:FlxSave = Save.system.bindSlot(slot);
    switch (saveData.status)
    {
      case EMPTY:
        ...
        switch (Save.system.fetchLegacySaveData())
        {
          case None:
            ...
            saveData.mergeData(gameSave.data, true);
            return gameSave;
      ...
      case ERROR(_): // Deprecated on newer Flixel, still used by older forks.
        return handleSaveDataError(slot);
      case BOUND(_, _):
        trace('[SAVE] Loaded existing save data in slot ${slot}.');
        var gameSave = SaveDataMigrator.migrate(saveData.data);
        saveData.mergeData(gameSave.data, true);
        return gameSave;
      case _:
        return handleSaveDataError(slot);
  ...
  public static function loadFromNewgrounds(onFinish:Void->Void):Void
  {
    ...
    {
      var saveData:FlxSave = Save.system.bindSlot(Constants.BASE_SAVE_SLOT);

      if (saveData.status != EMPTY)
      {
        // best i can do in case the NG file is corrupted or something along those lines
        var backupSlot:Int = Save.system.archiveBadSaveData(saveData.data);
        trace('[SAVE] Backed up current save data in case of emergency to $backupSlot!');
      }

      saveData.erase();
      saveData = Save.system.bindSlot(Constants.BASE_SAVE_SLOT); // forces regeneration of the file as erase deletes it
