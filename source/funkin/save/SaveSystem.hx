...
class SaveSystem
{
  public var current(default, null):FlxSave;
  ...
  public function bindSlot(slot:Int):FlxSave
  {
    current.bind(Constants.SAVE_NAME + slot, Constants.SAVE_PATH);
    return current;
  }

  public function clearSlot(slot:Int):Save
  {
    var saveData:FlxSave = bindSlot(slot);

    if (saveData.status == EMPTY) return new Save();
