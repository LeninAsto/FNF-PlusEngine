...
class PolymodHandler
{
  ...
  static function getModRoot():String
  {
    #if MODS_ALLOWED
    return Mods.getVSliceModsRoot();
    #else
    return MOD_FOLDER;
    #end
  }
  ...
  {
    FileUtil.createDirIfNotExists(getModRoot());
  }
  ...
  public static function loadEnabledMods():Void
  {
    ...
    loadModsByDir(getConfiguredModDirs());
  }
  ...
  public static function loadModsByDir(dirs:Array<String>):Void
  {
    ...
    for (mod in loadedModList)
    {
      ...
      loadedModDirs.push(getModDirName(mod));
  ...
  static function buildFileSystem():IFileSystem
  {
    polymod.Polymod.onError = PolymodErrorHandler.onPolymodError;
    return new VSliceNamespacedFileSystem(new ZipFileSystem({
      modRoot: getModRoot(),
      autoScan: true
    }));
  }
  ...
  static function buildImports():Void
  {
    ...
    Polymod.addImportAlias('FSSM', funkin.ui.FullScreenScaleMode);
    Polymod.addDefaultImport(funkin.audio.visualize.ABotAtlasSprite, 'ABotAtlasSprite');
    Polymod.addImportAlias('ABotAtlasSprite', funkin.audio.visualize.ABotAtlasSprite);
    Polymod.addImportAlias('funkin.audio.visualize.ABotAtlasSprite', funkin.audio.visualize.ABotAtlasSprite);
    Polymod.addDefaultImport(funkin.plus.PlusStateHelper, 'PlusStateHelper');
    Polymod.addImportAlias('funkin.plus.PlusStateHelper', funkin.plus.PlusStateHelper);
    ...
    Polymod.addImportAlias('flixel.math.FlxPoint', funkin.plus.vslice.VSliceFlxPoint);
    Polymod.addImportAlias('flixel.group.FlxSpriteGroup', funkin.plus.vslice.VSliceFlxSpriteGroup);
    Polymod.addImportAlias('flixel.util.FlxColor', funkin.plus.vslice.VSliceFlxColor);
    Polymod.addImportAlias('openfl.display.BlendMode', funkin.plus.vslice.VSliceBlendMode);
  ...
  {
    var modDirs:Array<String> = [for (i in getAllMods()) getModDirName(i)];
    return modDirs;
  }

  /**
   * Retrieve the enabled mod directories from Plus Engine when available.
   * VSlice's Save still exists, but Plus owns mod enable/order state.
   */
  public static function getConfiguredModDirs():Array<String>
  {
    #if MODS_ALLOWED
    return Mods.parseVSliceList().enabled.copy();
    #else
    return Save.instance.enabledModDirs.value.copy();
    #end
  }
  ...
  {
    var modDirs:Array<String> = getConfiguredModDirs();
    var modMetadata:Array<ModMetadata> = getAllMods();
    var enabledMods:Array<ModMetadata> = [];
    for (item in modMetadata)
    {
      if (modDirs.indexOf(getModDirName(item)) != -1)
      {
  ...
  static function getModDirName(mod:ModMetadata):String
  {
  ...
  public static function forceReloadAssets():Void
  {
    ...
    funkin.modding.PolymodHandler.loadModsByDir(loadedModDirs.length > 0 ? loadedModDirs.copy() : getConfiguredModDirs());
