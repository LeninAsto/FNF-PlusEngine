...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	{
		if (vortexIndicator == null || gridBg == null)
			return;
	...
	override function update(elapsed:Float)
	{
		...
		if (droppedFilePath != null || (FlxG.stage != null && FlxG.stage.window != null))
		{
			...
			var isDragging:Bool = FlxG.mouse.pressed
				&& (FlxG.mouse.screenY < 20
					|| FlxG.mouse.screenY > FlxG.height - 20
					|| FlxG.mouse.screenX < 20
					|| FlxG.mouse.screenX > FlxG.width - 20);
		...
		if (FlxG.sound.music != null)
		{
			if (PsychUIInputText.focusOn == null) // If not typing anything
			{
				...
				else if (touchPad.buttonLeft.justPressed
					|| FlxG.keys.justPressed.A != touchPad.buttonRight.justPressed
				...
				else if (touchPad.buttonUp.pressed
					|| FlxG.keys.pressed.W != touchPad.buttonDown.pressed
					|| FlxG.keys.pressed.S
					|| FlxG.mouse.wheel != 0)
				{
					if (FlxG.sound.music.playing)
		...
		if (PsychUIInputText.focusOn == null && lastFocus == null)
		{
			...
			else if (canContinue)
			{
				...
				else if (touchPad.buttonV.justPressed
					|| FlxG.keys.justPressed.Z != touchPad.buttonD.justPressed
					|| FlxG.keys.justPressed.X) // Decrease/Increase Zoom
				{
	...
	{
		if (selectedEventPanelText == null)
			return;
		...
		if (selectedNotes.length == 1 && selectedNotes[0].isEvent)
		{
			...
			for (note in stackedNotes)
				if (note != null && note.events != null)
					totalEvents += note.events.length;
		...
		else if (selectedNotes.length > 1)
		{
			...
			selectedEventPanelText.text = selectedEvents > 0 ? '$selectedEvents events are selected.\nSelect a single event to inspect its values.' : 'The current selection does not include any events.';
		}
		else
	...
	function onDropFile(path:String):Void
	{
		...
		openSubState(new BasePrompt('What type of file is this?', function(state:BasePrompt)
		{
			var btnY = 390;
